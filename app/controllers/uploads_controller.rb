class UploadsController < ApplicationController
  ...
  def avatar
    ...
    if avatar.save
      ...
      #current_user.info.update(avatar_id: avatar.id, avatar_url: avatar_url)
  ...
  def photo
    ...
    if avatar.save
      avatar_url = avatar.body.thumb.url
      #Product.first.update(avatar_url: avatar_url)
      product=Product.find_by_id(params[:photo_id])
      product.update(avatar_id: avatar.id, avatar_url: avatar_url)
      render json: Oj.dump([avatar_url])
    else
      render json: {msg: avatar.errors.full_messages.join(", ")}
    end
